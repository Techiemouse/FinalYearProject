...
public class TaggerVerbs {
	...
	public void taggIT(){
	String a="A labourer, named Doyle, has been arrested by the Liverpool police on a charge of having committed a fiendish outrage on his child";
	...
	System.out.println(tagged.replace("_VBN", "").replace("_VBZ", "").replace("_VBD", "").replace("_VBG", ""));
	//replacing the verbs tags with spaces
	String verbs =tagged.replace("_VBN", "").replace("_VBZ", "").replace("_VBD", "").replace("_VBG", "");
	//getting verb array
	getVerbs(verbs);
	}
	
	public ArrayList getVerbs(String text){
		
		ArrayList<String> verbList = new ArrayList<String>();
		//splitting string into words
		for(String word : text.split(" ")) {
			//System.out.println(word);
			//if words don't have the _ sign they will be verbs and will be added to the array
			if(!word.contains("_")){
			System.out.println(word);
			verbList.add(word);
			}
